...
# This file has been adapted to work with the OpenNeuro Alzheimer dataset
# --------------------------------------------------------
import os
import pickle
    ...
    for file in os.listdir(fetch_folder):
        if sub in file:
            ...
            try:
                ...
                with open("alz-process-error-files.txt", "a") as f:
                    f.write(file + "\n")
                continue
            window_size = 200
            for i in range(channeled_data.shape[1] // window_size):
                dump_path = os.path.join(
                    dump_folder, file.split(".")[0] + "_" + str(i) + ".pkl"
                )
                pickle.dump(
                    {"X": channeled_data[:, i * window_size : (i + 1) * window_size], "y": label},
                    open(dump_path, "wb"),
                )
