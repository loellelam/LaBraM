...
from shock.utils import preprocessing_set

savePath = Path('../output')
rawDataPath = Path('../data')
# group = rawDataPath.glob('*.cnt')
group = rawDataPath.glob('sub-*/eeg/*.set')
...
chunks = (19, rsfreq) # CHANGE CHANNEL NUMBER TO MATCH

dataset = h5Dataset(savePath, 'dataset')
for cntFile in group:
    print(f'processing {cntFile.name}')
    eegData, chOrder = preprocessing_set(cntFile, l_freq, h_freq, rsfreq)
